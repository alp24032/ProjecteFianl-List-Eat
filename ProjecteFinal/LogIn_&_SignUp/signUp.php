...
if(isset($_POST["userN_up"]) && isset($_POST["email_up"]) && isset($_POST["passw_up"])){
    ...
    if($existEmail !== NULL && $existusername !== NULL ){//comprobem que la query s'ha executat
		...
		if($infoEmail == NULL && $infoUsername == NULL){//en cas que la query sigui == NULL, significarà que l'Email i l'Username introduit no existeix.
			...
			echo "<script>alert('Error: Email [ $email_up ] already exists.')</script>";
        }else if($infoUsername != NULL){
			echo "<script>alert('Error: Username [ $userN_up ] already exists.')</script>";
		}else{
			echo "<script>alert('Error: Username [ $userN_up ] and Email [ $email_up ] already exists.')</script>";
		}
            
    }else {
		echo "<script>alert('Error a hora d\'xecutar la consulta.')</script>";
	}
}
				...
				<div class="col-md-12 col-lg-10">
					<div class="wrap d-md-flex">
						...
						<div class="login-wrap p-4 p-lg-5">
							...
							<form id="signUp_form" name="signUp_form" action="signUp.php" method="post" class="signin-form">
